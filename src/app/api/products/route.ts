...
export async function POST(req: Request) {
  try {
    const { name, quantity, price, cost, departmentId, supplierId } =
      await req.json();
    ...
    if (quantity <= 0) {
      return new Response("Quantity must be greater than zero", {
        status: 400,
      });
    }

    if (price <= 0 || cost <= 0) {
      return new Response("Price and cost must be greater than zero", {
        status: 400,
      });
    }
